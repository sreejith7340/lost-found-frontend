...
const { sendClaimApprovalEmail, sendClaimNotificationToReporter, sendClaimRejectionEmail } = require('../config/email');
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const foundRejected = await FoundItem.countDocuments({ status: 'rejected' });

        // Total claimed items
        const totalClaimedItems = lostClaimed + foundClaimed;
        ...
        res.status(200).json({
            ...
            stats: {
                ...
                byStatus: {
                    ...
                    found: { pending: foundPending, approved: foundApproved, claimed: foundClaimed, rejected: foundRejected }
                },
                lostByCategory,
                recentLost,
                recentFound
...
const updateClaimStatus = async (req, res) => {
    try {
        ...
        // If approved, update the item status to claimed and send emails
        if (status === 'approved') {
            const Model = claim.itemType === 'lost' ? LostItem : FoundItem;
            const item = await Model.findByIdAndUpdate(claim.itemId, { status: 'claimed' });
            ...
            if (item) {
                const emailResult = await sendClaimApprovalEmail(
                    ...
                    {
                        name: claim.itemType === 'lost' ? item.reporter.name : item.finder.name,
                        email: claim.itemType === 'lost' ? item.reporter.email : item.finder.email,
                        phone: claim.itemType === 'lost' ? item.reporter.phone : item.finder.phone,
                ...
                if (emailResult.success) {
                    await sendClaimNotificationToReporter(
                        {
                            name: claim.itemType === 'lost' ? item.reporter.name : item.finder.name,
                            email: claim.itemType === 'lost' ? item.reporter.email : item.finder.email
                        },
        ...
        // If rejected, send rejection email to claimant
        if (status === 'rejected') {
            try {
